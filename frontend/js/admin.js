...
document.getElementById('authform').addEventListener('submit', function(e) {
    ...
    .then(() => { // if we don't fail
        msgElement.innerHTML = 'operation completed. <a href="/">return</a>';
        msgElement.style.color = '#006b0c';
        msgElement.style.display = 'block';
    })
    .catch(err => { // if we fail
        console.error('failed to send form data:', err);
        msgElement.innerHTML = err === 'HTTP 401' ? 'authentication failed <a href="/">return</a>' : `operation failed (${err}). <a href="/">return</a>`; // print special message for 401, otherwise just a generic message
        msgElement.style.color = '#9f0000';
        msgElement.style.display = 'block';
    });
});
